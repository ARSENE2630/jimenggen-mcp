...
// 生成视频配置
function generateVideoConfig(frames, aspect_ratio) {
    return {
        frames: 121,
        aspect_ratio: '16:9'
    };
...
// 生成数字人配置
function generateDigitalHumanConfig(avatarStyle, emotion, action) {
    return {
...
// 注册图生图工具
server.tool("image-to-image", "使用即梦AI图生图模型基于参考图片生成新图片", {
...
}, async ({ prompt, imageUrl, ratio, localPath }) => {
    // 检查必需参数是否存在
    if (!prompt || !ratio) {
        return {
            content: [
...
// 注册视频生成工具
server.tool("generate-video", "使用即梦AI视频生成模型生成短视频，需要先创建数字形象", {
...
}, async ({ prompt, frames, aspect_ratio }) => {
    // 检查必需参数是否存在
...
// 注册数字人生成工具
server.tool("generate-digital-human", "使用即梦AI数字人模型生成数字人内容", {
...
}, async ({ prompt, avatarStyle, emotion, action }) => {
    // 检查必需参数是否存在
...
// 注册图片换装工具
server.tool("image-dressing", "使用即梦AI图片换装模型为模特更换服装", {
    ...
    if (localPathArray && reqImageStoreType === 0) {
        const localPathArrayBase64 = await Promise.all(JSON.parse(localPathArray).map(async (path) => await readFileAsBase64(path)));
